...
import type { DiscriminatedWebhookPayload } from "./types";
...
const updateUserSubscription = async (
  ...
  await db(env).update(user).set({ subscriptionId }).where(eq(user.id, userId));
};
...
paymentRouter.post("/webhook", async (c) => {
  ...
  const payload = JSON.parse(body) as DiscriminatedWebhookPayload<{
    email: string;
  }>;
